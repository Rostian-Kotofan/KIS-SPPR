...
export default function KPIDashboard({ projects, financials, resources }: KPIDashboardProps) {
  ...
  const pieData = [
    { name: "Операционные издержки (OpEx)", value: financials.opEx, color: "#6366f1" },
  ...
  // Динамика квартальных доходов (Line Chart)
  const lineData = [
    { name: "I Кв 2025", Выручка: 11200000, Расходы: 8900000, Прибыль: 1840000 },
    { name: "II Кв 2025", Выручка: 12500000, Расходы: 9800000, Прибыль: 2160000 },
    { name: "III Кв 2025", Выручка: 13800000, Расходы: 10500000, Прибыль: 2640000 },
    { name: "IV Кв 2025", Выручка: 14200000, Расходы: 11100000, Прибыль: 2480000 },
    { name: "I Кв 2026", Выручка: financials.revenue, Расходы: totalCosts, Прибыль: netProfit }
  ];
  ...
  return (
    <div id="kpi-dashboard-root" className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div id="card-profit" className="bg-white p-5 rounded-xl border border-slate-100 shadow-xs flex items-center justify-between">
          <div className="space-y-1">
            ...
            <span className="text-xs text-indigo-600 font-medium">После налога {financials.taxRate}%</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-indigo-900 text-white rounded-xl p-6 shadow-xs flex flex-col justify-between space-y-4">
          <div>
            <span className="text-xs text-indigo-200 uppercase font-bold tracking-wider">Корпоративный статус</span>
          ...
          <div className="border-t border-indigo-800 pt-4 grid grid-cols-3 gap-2 text-center">
            <div>
              <div className="text-lg font-bold font-mono">{activeProjectsCount}</div>
              <div className="text-[10px] text-indigo-200">В работе</div>
            </div>
            <div>
              <div className="text-lg font-bold font-mono">{completedProjectsCount}</div>
              <div className="text-[10px] text-indigo-200">Сдано</div>
            </div>
            <div>
              <div className="text-lg font-bold font-mono">{resources.length}</div>
              <div className="text-[10px] text-indigo-200">Сотрудников</div>
            </div>
          </div>
        </div>
          ...
          <div className="h-80 w-full text-xs">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={lineData} margin={{ top: 10, right: 30, left: 10, bottom: 5 }}>
                ...
                <Line type="monotone" dataKey="Выручка" stroke="#6366f1" strokeWidth={3} activeDot={{ r: 8 }} />
